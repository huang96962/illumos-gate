 ...
 */
int
cfb_encrypt_contiguous_blocks(cfb_ctx_t *ctx, char *data, size_t length,
    ...
    void (*xor_block)(uint8_t *, uint8_t *))
{
	...
	do {
		...
		if (out == NULL) {
			...
			if ((ctx->cbc_flags & CFB_MAC_MODE) == 0 &&
...
/* ARGSUSED */
int
cfb_decrypt_contiguous_blocks(cfb_ctx_t *ctx, char *data, size_t length,
    crypto_data_t *out, size_t block_size,
...
int
cfb_init_ctx(cfb_ctx_t *cbc_ctx, char *param, size_t param_len,
...
/* ARGSUSED */
static void *
cfb_cmac_alloc_ctx(int kmflag, uint32_t mode)
{
	cfb_ctx_t *cbc_ctx;
	uint32_t modeval = mode & (CFB_MODE|CFB_MAC_MODE);

	/* Only one of the two modes can be set */
	VERIFY(modeval == CFB_MODE || modeval == CFB_MAC_MODE);

#ifdef _KERNEL
	if ((cbc_ctx = kmem_zalloc(sizeof (cfb_ctx_t), kmflag)) == NULL)
#else
	if ((cbc_ctx = calloc(1, sizeof (cfb_ctx_t))) == NULL)
#endif
		return (NULL);
...
{
	return (cfb_cmac_alloc_ctx(kmflag, CFB_MODE));
}
...
void *
cfb_mac_alloc_ctx(int kmflag)
{
	return (cfb_cmac_alloc_ctx(kmflag, CFB_MAC_MODE));
}
 ...
 */
int
cfb_mac_init_ctx(cfb_ctx_t *cbc_ctx, size_t block_size)
{
	/*
	...
	cbc_ctx->cbc_lastp = (uint8_t *)&cbc_ctx->cbc_iv[0];
	cbc_ctx->cbc_flags |= CFB_MAC_MODE;
 ...
 */
static uint8_t
cfb_mac_left_shift_block_by1(uint8_t *block, size_t block_size)
{
	uint8_t carry = 0, old;
	size_t i;
 ...
 */
int
cfb_mac_mode_final(cfb_ctx_t *cbc_ctx, crypto_data_t *out,
    ...
    void (*xor_block)(uint8_t *, uint8_t *))
{
	...
	if (cfb_mac_left_shift_block_by1(buf, block_size))
