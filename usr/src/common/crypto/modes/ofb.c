 ...
 */
int
ofb_encrypt_contiguous_blocks(ofb_ctx_t *ctx, char *data, size_t length,
    ...
    void (*xor_block)(uint8_t *, uint8_t *))
{
	...
	do {
		...
		if (out == NULL) {
			...
			if ((ctx->cbc_flags & OFB_MAC_MODE) == 0 &&
			...
			encrypt(ctx->cbc_keysched, lastp, lastp);
			/*
			 * lastp 加密后，即成为新的lastp。blockp则利用OTHER来缓存结果
			 */
			copy_block(blockp, (uint8_t *)OTHER((uint64_t *)lastp, ctx));
			blockp = (uint8_t *)OTHER((uint64_t *)lastp, ctx);
			xor_block(lastp, blockp);
			ctx->cbc_lastp = lastp;
			...
			if ((ctx->cbc_flags & OFB_MAC_MODE) == 0) {
				...
				if (out_data_1_len == block_size) {
					copy_block(blockp, out_data_1);
				} else {
					bcopy(blockp, out_data_1,
					    out_data_1_len);
					if (out_data_2 != NULL) {
	...
	/*
	 * Save the out block temporaryly for SM4
	 */
	if ((ctx->cbc_flags & OFB_MAC_MODE) != 0) {
		copy_block(blockp, (uint8_t *)ctx->cbc_remainder);
	}
...
out:
	 ...
	 */
	if ((ctx->cbc_flags & OFB_MAC_MODE) != 0) {
		copy_block((uint8_t *)ctx->cbc_remainder, (uint8_t *)ctx->cbc_lastblock);
	}
...
/* ARGSUSED */
int
ofb_decrypt_contiguous_blocks(ofb_ctx_t *ctx, char *data, size_t length,
    crypto_data_t *out, size_t block_size,
    ...
    void (*xor_block)(uint8_t *, uint8_t *))
{
	...
	do {
		...
		if (out != NULL) {
			/*
			 * lastp 解密后，即成为新的lastp。blockp则利用OTHER来缓存结果
			 */
			copy_block(blockp, (uint8_t *)OTHER((uint64_t *)lastp, ctx));
			blockp = (uint8_t *)OTHER((uint64_t *)lastp, ctx);
		}

		decrypt(ctx->cbc_keysched, lastp, lastp);
...
int
ofb_init_ctx(ofb_ctx_t *cbc_ctx, char *param, size_t param_len,
...
/* ARGSUSED */
static void *
ofb_cmac_alloc_ctx(int kmflag, uint32_t mode)
{
	ofb_ctx_t *cbc_ctx;
	uint32_t modeval = mode & (OFB_MODE|OFB_MAC_MODE);

	/* Only one of the two modes can be set */
	VERIFY(modeval == OFB_MODE || modeval == OFB_MAC_MODE);

#ifdef _KERNEL
	if ((cbc_ctx = kmem_zalloc(sizeof (ofb_ctx_t), kmflag)) == NULL)
#else
	if ((cbc_ctx = calloc(1, sizeof (ofb_ctx_t))) == NULL)
#endif
		return (NULL);
...
void *
ofb_alloc_ctx(int kmflag)
{
	return (ofb_cmac_alloc_ctx(kmflag, OFB_MODE));
}
...
void *
ofb_mac_alloc_ctx(int kmflag)
{
	return (ofb_cmac_alloc_ctx(kmflag, OFB_MAC_MODE));
}
 ...
 */
int
ofb_mac_init_ctx(ofb_ctx_t *cbc_ctx, size_t block_size)
{
	/*
	...
	cbc_ctx->cbc_lastp = (uint8_t *)&cbc_ctx->cbc_iv[0];
	cbc_ctx->cbc_flags |= OFB_MAC_MODE;
 ...
 */
static uint8_t
ofb_mac_left_shift_block_by1(uint8_t *block, size_t block_size)
{
	uint8_t carry = 0, old;
	size_t i;
 ...
 */
int
ofb_mac_mode_final(ofb_ctx_t *cbc_ctx, crypto_data_t *out,
    ...
    void (*xor_block)(uint8_t *, uint8_t *))
{
	...
	if (ofb_mac_left_shift_block_by1(buf, block_size))
